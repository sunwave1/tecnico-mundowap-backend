...
class WorkdaysController extends ApiController
{
    ...
    public function close(string $date): void {
        ...
        try {
            ...
            $connection->transactional(function() use ($date, &$movedCount, &$totalToMove) {
                ...
                $movedCount = $this->movePendingVisits($entitiesPendingVisit->toArray(), $currentDate);
    ...
    private function movePendingVisits(array $pendingVisits, \DateTime $startDate): int {
        ...
        foreach($pendingVisits as $visit) {
            ...
            $freeDate = $this->getNextFreeDate($currentTargetDate, $visit->duration);
    ...
    private function getNextFreeDate(\DateTime $startDate, int $visitDuration): ?\DateTime {
